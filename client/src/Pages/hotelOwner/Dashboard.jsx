        ...
        <tbody className='text-sm'>
         {dashboardData.bookings.map((item,index)=> (
            <tr key={index}>
                ...
                <td className='py-3 px-4 text-gray-600 border-t border-gray-300 max-sm:hidden'>
                    {item.room.roomType}
                </td>

                <td className='py-3 px-4 text-gray-600 border-t text-center border-gray-300'>
                   ₹ {item.totalPrice}
                </td>
                
                <td className='py-3 px-4  border-t text-center border-gray-300'>
                  <button className={`py-1 px-3 text-xs rounded-full mx-auto ${item.isPaid ? "bg-green-200 text-green-700" : "bg-amber-200 text-yellow-700"} `}>
